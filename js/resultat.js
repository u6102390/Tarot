...
document.addEventListener('DOMContentLoaded', () => {
	...
	if (titleEl) titleEl.textContent = origName;
		const [l1, l2] = getTwoLines(first.description, origName);
	...
	if (title2El) title2El.textContent = origName2;
		const [s1, s2] = getTwoLines(second.description, origName2);
	...
	if (title3El) title3El.textContent = origName3;
		const [t1, t2] = getTwoLines(third.description, origName3);
